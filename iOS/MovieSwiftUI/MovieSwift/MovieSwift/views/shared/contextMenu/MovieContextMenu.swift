...
struct MovieContextMenu: ConnectedView {
    struct Props {
        ...
        let customLists: [String: CustomList]
        let onAddToWishlist: () -> Void
        let onAddToSeenList: () -> Void
        let onAddToCustomList: (String) -> Void
    }
...
extension MovieContextMenu {
    func map(state: AppState, dispatch: @escaping DispatchFunction) -> Props {
        ...
        return Props(isInWishlist: isInWishlist,
                     ...
                     customLists: lists as! [String : CustomList], //probably runtime crash
                     onAddToWishlist: {
            ...
            let isIn = (lists[list] as? CustomList)?.movies.contains(self.movieId) == true
...
extension MovieContextMenu {
    ...
    private func onAddToCustomList(list: String, isIn: Bool, dispatch: DispatchFunction) {
        if isIn {
