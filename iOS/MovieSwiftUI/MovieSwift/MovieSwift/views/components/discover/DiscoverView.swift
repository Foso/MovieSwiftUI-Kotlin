...
struct DiscoverView: ConnectedView {
    ...
    @State private var previousMovie: String? = nil
    ...
    func map(state: AppState, dispatch: @escaping DispatchFunction) -> DiscoverViewProps {
        return DiscoverViewPropsKt.discoverViewPropsMapper()(state, dispatch)
    }
    ...
    private func draggableCoverEndGestureHandler(props: DiscoverViewProps, handler: DraggableCover.EndState) {
    ...
    private func fetchRandomMovies(props: DiscoverViewProps, force: Bool, filter: DiscoverFilter?) {
        if props.movies.count < 10 || force {
            props.dispatch(movieActions.fetchRandomDiscover(filter: filter))
        }
    }
    
    // MARK: Body views
    private func filterView(props: DiscoverViewProps) -> some View {
    ...
    private func actionsButtons(props: DiscoverViewProps) -> some View {
        ZStack(alignment: .center) {
            if props.currentMovie != nil {
                Text(props.currentMovie!.userTitle(appUserDefaults: appUserDefaults))
    ...
    private func draggableMovies(props: DiscoverViewProps) -> some View {
        ForEach(props.movies, id: \.self) { id in
            Group {
                if props.isLastMovieInList(id: id) {
                    ...
                    DiscoverCoverImage(imageLoader: ImageLoaderCache.shared.loaderFor(path: props.posters[id],
                                                                                      size: .medium))
                        .scaleEffect(1.0 - CGFloat(Int(props.reversedMoviesFirstIndexOf(id: id))) * 0.03 + CGFloat(self.scaleResistance()))
                        .padding(.bottom, CGFloat(Int(props.reversedMoviesFirstIndexOf(id: id)) * 16) - self.dragResistance())
    ...
    func body(props: DiscoverViewProps) -> some View {
        ZStack(alignment: .center) {
        ...
        .onAppear {
            ...
            self.fetchRandomMovies(props: props, force: false, filter: props.filter)
            props.dispatch(movieActions.fetchGenres())
        }
    }
}
