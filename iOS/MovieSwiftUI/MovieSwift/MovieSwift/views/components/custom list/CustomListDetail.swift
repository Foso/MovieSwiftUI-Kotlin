...
final class CustomListSearchMovieTextWrapper: SearchTextObservable {
    override func onUpdateTextDebounced(text: String) {
        store.dispatch(movieActions.fetchSearch(query: text, page: 1))
    }
}
...
struct CustomListDetail : View {
    ...
    private var list: CustomList {
        store.state.moviesState.customLists[listId] as! CustomList
    }
    
    private var movies: [String] {
        list.sortedMoviesIds(by: selectedMoviesSort, state: store.state)
    }
        
    private var searchedMovies: [String]? {
        store.state.moviesState.search(searchText: searchTextWrapper.searchText)
    }
            ...
            if isSearching {
                Button(action: {
                    ...
                    if !self.selectedMovies.isEmpty {
                        self.store.dispatch(MoviesActions.AddMoviesToCustomList(list: self.listId,
                                                                                        movies: self.selectedMovies.map{ $0 }))
                        self.selectedMovies = Set<String>()
                    }
                }) {
            ...
            Group {
                if isSearching {
                    ...
                    ForEach(movies, id: \.self) { movie in
                        ...
                        self.store.dispatch(MoviesActions.RemoveMovieFromCustomList(list: self.listId, movie: self.movies[index.first!]))
                    }
                }
            }
...
struct CustomListDetail_Previews : PreviewProvider {
    static var previews: some View {
        NavigationView {
            CustomListDetail(listId: (sampleStore.state.moviesState.customListsList[0]).id)
