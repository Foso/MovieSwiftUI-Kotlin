...
struct CustomListForm : View {
    ...
    @State var listMovieCover: String?
    
    let editingListId: String?
    
    private var searchedMovies: [String] {
        return store.state.moviesState.search(searchText: searchTextWrapper.searchText)! //?.map{ $0 } ?? []
    }
    ...
    private var buttonsSection: some View {
        Section {
            Button(action: {
                let newList = CustomList(id: String(Int.random(in: self.store.state.moviesState.customLists.count ..< 1000^3)),
                                         name: self.listName,
                                         cover: self.listMovieCover,
                                         movies: [])
                if let id = self.editingListId {
                    self.store.dispatch(MoviesActions.EditCustomList(list: id,
                                                                             title: self.listName,
                                                                             cover: self.listMovieCover))
                } else {
                    self.store.dispatch(MoviesActions.AddCustomList(list: newList))
                }
    ...
    var body: some View {
        ...
        .onAppear() {
            if let id = self.editingListId,
                let list = self.store.state.moviesState.withListId(listId: id) {
