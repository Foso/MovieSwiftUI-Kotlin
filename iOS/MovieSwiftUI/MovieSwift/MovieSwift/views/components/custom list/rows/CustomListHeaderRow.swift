...
struct CustomListHeaderRow : View {
    ...
    let listId: String
    var list: CustomList {
        return store.state.moviesState.customLists[listId]! as! CustomList
    }
    var coverBackdropMovie: Movie? {
        guard let id = list.cover else {
            return nil
        }
        return store.state.moviesState.withMovieId(movieId: id)
    }
...
struct CustomListHeaderRow_Previews : PreviewProvider {
    static var previews: some View {
        CustomListHeaderRow(sorting: .constant(MoviesSort.byaddeddate), listId: "0").environmentObject(sampleStore)
    }
}
#endif
